...
run; 
data PercentData2;
     merge PercentData(keep=physician Questions &items &nitems &vitems    date2 where=(physician=&physician))
           PercentData(keep=physician &Sitems &SNitems &SVitems date2 where=(physician=99955))
           PercentData(keep=physician &Ritems &RNitems &RVitems date2 where=(physician=99900))
           ; by date2; 
	 physician=&physician;
...
data PercentData2; set PercentData2;
     retain obs 0;
	 obs+1;
	 if question=4 then do; if phcc=. then delete; end;
	 if question=7 then do; if nucc=. then delete; end;
